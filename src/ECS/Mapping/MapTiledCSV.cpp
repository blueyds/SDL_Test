#include "Mapping/MapTiledCSV.hpp"
...
ECS::MapTiledCSV::~MapTiledCSV(){};


void ECS::MapTiledCSV::loadMap(std::string path, int sizeX, int sizeY) {
  std::fstream mapFile;
  mapFile.open(path);
      ...
      while (i < count) {
        ...
        if (temp >= 0) {
          ...
          if (digit >= 0) {
            ...
            tileId += digit;
            RpgGame::addTile(tileId, row, column);
            column += 1;
            tileId = -1;
